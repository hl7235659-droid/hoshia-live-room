...
- Accepts `POST /live-room/generate` for live-room replies, `POST /live-room/news` for optional daily news memory ingestion, and `POST /live-room/memory/debug-recall` for internal memory checks.
...
- Skips duplicate viewer memories before writing and filters or deletes expired daily news memories.
...
LivingMemory integration is disabled by default. Enable it with `livingmemory_enabled` only after the LivingMemory plugin is installed and its data directory is backed up. Viewer memories use `live-room:<room_id>:user:<user_id>` sessions, and daily news uses `live-room:<room_id>:news`. The debug recall endpoint requires the same bearer token and is intended only for internal deployment checks.
