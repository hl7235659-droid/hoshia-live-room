...
class LiveRoomBridgePlugin(Star):
    ...
    def _build_short_term_context(self, context_summary: str, recent_context: Any) -> str:
        ...
        if sections:
            sections.insert(
                0,
                "模块上下文是给 Hoshia 找话题用的材料，不是要逐字段复述的后台报告。"
                "遇到 safe news、diary、music、game 或 contact-window hooks 时，优先转成一句个人反应、具体物件、吐槽或可接话的问题；"
                "不要只说状态标签，也不要透露字段名、接口、路径、密钥或内部配置。"
            )
    ...
    def _format_module_prompt_context(self, module_context: list[dict[str, Any]], module_events: list[dict[str, Any]]) -> str:
        ...
        if not sections:
            return ""
        return (
            "当前联系窗口状态参考。只把它当作你能感知到的状态和网友行为信号。"
    ...
    def _build_proactive_idle_instruction(self) -> str:
        return """
主动开口提示：
- 现在至少有一位特殊网友在线，联系窗口刚刚比较安静；你可以先轻轻开一个话题。
