...
export function createLiveAiReplyController({
  ...
  const newsService = hoshiaNewsService;
  ...
  function formatLiveRoomBatchPrompt(batch, lifeMemoryPacket = [], { activeContext = {}, contextPolicy = {}, moduleContext = null, moduleEvents = null } = {}) {
    ...
    const hostLifeContextLines = buildHostLifeContext({
      ...
      dailyCanonContext: hoshiaDailyCanonService?.buildContext?.(batch[0]?.session, { now: new Date(), create: true }),
      newsService,
      interestContext: hoshiaInterestSystem?.buildContext?.(batch[0]?.session, { now: new Date() }),
      currentVisualState: buildCurrentCharacterSnapshot()
    });
  ...
  return {
    ...
    replyTargets,
    roomAiSession,
    getPendingReplyCount: () => pendingReplyBatch.length,
    getReplyBatchRunning: () => replyBatchRunning
  };
}
