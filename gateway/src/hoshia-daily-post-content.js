...
function createHoshiaDailyPostCreatedEvent(post, state, {
  roomId = "",
...
function createHoshiaDailyPostCharacterEvent(post, session = null, {
...
function buildDailyPostContent(state = {}, now = new Date(), timeZone = defaultTimeZone, context = {}) {
...
function buildNewsTopicPostContent(topic = {}, state = {}, now = new Date(), timeZone = defaultTimeZone) {
...
function dayKeyFor(value = new Date(), timeZone = defaultTimeZone) {
...
function normalizeDailyPostMinimum(value) {
  const number = Math.floor(Number(value));
  if (!Number.isFinite(number)) return defaultDailyMin;
  return Math.max(1, Math.min(number, defaultDailyMax));
}

function normalizeDailyPostMaximum(value, minimum = defaultDailyMin) {
  const number = Math.floor(Number(value));
