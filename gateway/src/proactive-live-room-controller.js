export function createProactiveLiveRoomController(deps) {
  ...
  async function handleProactiveReplyCheck() {
    const decision = shouldRunProactiveReply({
      ...
      pendingReplyCount: getPendingReplyCount(),
      replyBatchRunning: getReplyBatchRunning()
    });
  ...
  async function sendHoshiaClawProactiveLiveReply(idleMs, liveDecision = {}) {
    ...
    try {
      ...
      // 组装活人感topic hooks
      const lifeHooks = [];
      const dailyCanon = hoshiaDailyCanonService?.buildContext?.(null, { now: new Date(), create: true });
      if (dailyCanon?.enabled) {
        if (dailyCanon.active_event) lifeHooks.push(`我现在正在：${dailyCanon.active_event.summary || dailyCanon.active_event.title}`);
        if (dailyCanon.active_event?.chat_hooks?.[0]) lifeHooks.push(dailyCanon.active_event.chat_hooks[0]);
      }
      if (hoshiaNewsService?.enabled()) {
        const featured = hoshiaNewsService.featuredTopic();
        if (featured && !featured.high_risk) lifeHooks.push(`最近的话题：${featured.title}，可以说：${featured.conversation_starter || ""}`);
      }
      const interestCtx = hoshiaInterestSystem?.buildContext?.(null, { now: new Date() });
      if (interestCtx?.enabled && interestCtx.current_state?.length) {
        lifeHooks.push(...interestCtx.current_state.slice(0,2));
      }
      const visualState = typeof currentCharacterState === "function" ? currentCharacterState() : null;
      if (visualState) lifeHooks.push(`现在状态：${visualState.mood || "平静"}，精力${Math.round(visualState.energy || 50)}%，在${visualState.activity || "发呆"}`);

      const prompt = buildProactiveLivePrompt({
        idleMs,
        onlineCount: roomInfo().online,
        unansweredCount: proactiveReplyState.unansweredCount,
        topicHooks: [...proactiveTopicHooks({ moduleContext, moduleEvents, recentMessages }), ...lifeHooks].filter(Boolean),
        recentMessages,
        characterSnapshotContext,
        hoshiaPersonaPrompt
      });
  ...
  function firstActiveSession() {
    for (const [ws, session] of sockets.entries()) {
      if (ws.readyState === webSocketOpen && activeUserConnections.has(session.user_id)) return session;
    }
    return null;
  }
